...
The mean number of steps per day is `r prettyNum(meansteps,big.mark=",",scientific=F)` and the meadian is `r prettyNum(medsteps,big.mark=",",scientific=F)`, 
...
maximum of `r round(maxsteps,2)` steps are logged in the interval beginning
at `r strftime(maxint,"%H:%M")`.
...
In total, we count `r numNA` NA values. To address these missing values, we will 
...
The mean number of steps per day after modification is `r prettyNum(modmeansteps,big.mark=",",scientific=F)` and 
the meadian is `r prettyNum(modmedsteps,big.mark=",",scientific=F)`, indicated by the dashed red line and the solid 
